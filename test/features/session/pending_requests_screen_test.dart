  ...
  await tester.pumpWidget(
    ProviderScope(
      ...
      child: MaterialApp(
        ...
        home: const PendingRequestsScreen(),
      ),
    ),
  );
...
void main() {
  testWidgets('shows pending requests list and handles approve/reject decisions', (
    tester,
  ) async {
  ...
  testWidgets('renders empty pending requests UI when list is empty', (
    ...
    expect(find.text('Không có yêu cầu tham gia đang chờ'), findsOneWidget);
    expect(
      find.text('Các yêu cầu xin tham gia từ người chơi khác sẽ hiển thị tại đây.'),
      findsOneWidget,
    );
  });
}
